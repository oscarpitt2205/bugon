...
final class GoogleQrUrl
{
    ...
    public static function generate(string $accountName, string $secret, string $issuer = null, int $size = 200): string
    {
        ...
        $otpauthString = urlencode(sprintf($otpauthString, $label, $secret, urlencode($issuer)));
        $url = sprintf(
            'http://chart.apis.google.com/chart?chs=%1$dx%1$d&chld=L|0&choe=UTF-8&cht=qr&chl=%2$s',
            $size,
            $otpauthString
        );
